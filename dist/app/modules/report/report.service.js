...
// stock status
const stockStatus = (filters, paginationOptions) =>
  __awaiter(void 0, void 0, void 0, function* () {
    const { id } = filters;
...
// summary report
const vehicleSummaryReport = (filters, paginationOptions) =>
  __awaiter(void 0, void 0, void 0, function* () {
    ...
    const expenseWhereConditions =
      othersAndConditions.length > 0 ? { AND: othersAndConditions } : {};
    const maintenanceWhereConditions =
      othersAndConditions.length > 0 ? { AND: othersAndConditions } : {};
    const paperWhereConditions =
      othersAndConditions.length > 0 ? { AND: othersAndConditions } : {};
...
const tripSummaryGroupByMonthYear = () =>
  __awaiter(void 0, void 0, void 0, function* () {
    const result = yield prisma_1.default
      .$queryRaw`SELECT EXTRACT(YEAR FROM "startDate") AS year, EXTRACT(MONTH FROM "startDate") AS month,  SUM(amount) AS total_amount FROM trips GROUP BY year, month ORDER BY year, month`;
...
const fuelSummaryGroupByMonthYear = () =>
  __awaiter(void 0, void 0, void 0, function* () {
    const result = yield prisma_1.default
      .$queryRaw`SELECT EXTRACT(YEAR FROM date) AS year, EXTRACT(MONTH FROM date) AS month, SUM(quantity) AS total_quantity, SUM(amount) AS total_amount FROM fuels GROUP BY year, month ORDER BY year, month`;
    const formattedResult = result.map(row => ({
      year: row.year,
      month: row.month,
